...
parser.add_argument("--numcells", metavar = "numcells", type = int, default = 100)
parser.add_argument("--pcadim", metavar = "pcadim", type = int, default = 10)
parser.add_argument("--lamda", metavar = "lamda", type = float, default = 1e-3)
parser.add_argument("--eps_df", metavar = "eps_df", type = float, default = 0.05)
...
props_perturb = [normalise(p * np.random.uniform(0, 1, size = len(p))) for p in props]
...
model_ng.solve_lbfgs(steps = args.steps_ng, max_iter = 25, lr = 1, history_size = 10, line_search_fn = 'strong_wolfe', factor = 2, tol = 1e-5, retry_max = 0)
...
model_g = gwot.models.OTModel(tsdata, lamda = args.lamda,
        ...
        m_i = m_i,
        g_i = g_i.cuda(),
        kappa = torch.from_numpy(25/tsdata.dt).cuda(), 
        c_scale = torch.from_numpy(c_means).cuda(),
        c_scale_df = torch.from_numpy(c_means_self).cuda(),
        growth_constraint="KL",
        pi_0 = "uniform", 
